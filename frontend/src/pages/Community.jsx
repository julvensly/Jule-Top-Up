 ...
 const rechercherCommunity = 
  async () => {
    ...
    try { setLoading(true); setErreur(""); 
      setCommunity(null); const response = await 
      api.get(`/api/community/${id}`); 
...
navigate(`/community/${community.slug}`);
  ...
  return ( <div className="auth-page"> <div 
      className="back-button">
        <button onClick={() => navigate(-1)}> <ArrowLeft 
          size={22} />
        </button> </div> <div className="auth-card"> 
        <div className="auth-icon">
          <Users size={30} /> </div> <h1>Community</h1> 
        <p className="auth-subtitle">
          Entrez l'identifiant de votre Community </p> 
        <div className="input-group">
          <Search size={20} /> <input type="text" 
            placeholder="Community ID" 
            value={communityId} onChange={(e) => {
