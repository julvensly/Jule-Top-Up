...
const registerUser = async (req, res) => { try {
 ...
 message: "Le mot de  passe doit contenir 8 caractères avec une  majuscule, une minuscule, un chiffre et un caractère spécial",
      });
    }
 ...
 const user = await User.create({  nom, email, password: hashedPassword,
    ...
    res.status(201).json({ message: "Votre compte a été créé avec succès", user: {
